...
export default function Notes(): React.JSX.Element {
    ...
    async function handleDuplicate(
        ...
        try {
            ...
            if (data.status === ResponseStatus.GOOD) {
                ...
                setMessage(
                    data.message || "Errore durante la duplicazione della nota"
                );
            }
        } catch (e) {
            setMessage("Impossibile raggiungere il server");
    ...
    async function handleDelete(
        ...
        try {
            ...
            if (data.status === ResponseStatus.GOOD) {
                ...
                setMessage(
                    data.message || "Errore durante l'eliminazione della nota"
                );
            }
        } catch (e) {
            setMessage("Impossibile raggiungere il server");
    ...
    function sortBy(method: Order): void {
        switch (method) {
            ...
            case Order.DATE:
                setNoteList(
                    [...noteList].sort((a, b) => {
                        ...
                        return a.updatedAt
                            .toString()
                            .localeCompare(b.updatedAt.toString());
                    })
                );
                break;
            ...
            case Order.LENGTH:
                setNoteList(
                    [...noteList].sort((a, b) => a.text.length - b.text.length)
                );
                break;
            case Order.LENGTH_DESC:
                setNoteList(
                    [...noteList].sort((a, b) => b.text.length - a.text.length)
                );
                break;
