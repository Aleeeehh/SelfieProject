...
async function getStatusForActivity(activity: Activity): Promise<ActivityStatus> {
	...
	const prevActivities = await ActivitySchema.find({ next: activity.id }).lean();
...
router.put("/:id", async (req: Request, res: Response) => {
	...
	try {
		...
		if (
			...
			console.log(
				"Invalid body: 'title', 'description', 'deadline', 'completed', 'accessList', 'active', 'abandoned' or 'reactivated' required, nothing to update"
			);
			return res.status(400).json({
				status: ResponseStatus.BAD,
				message:
