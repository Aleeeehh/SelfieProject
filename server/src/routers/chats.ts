...
router.get("/", async (req: Request, res: Response) => {
	try {
		...
		for (const foundChat of foundChats) {
			...
			const firstUser = await UserSchema.findById(foundChat.firstUserId).lean();
			if (!firstUser) {
				console.log("User not found: " + foundChat.firstUserId);
				continue;
			}

			const secondUser = await UserSchema.findById(foundChat.secondUserId).lean();
			if (!secondUser) {
...
router.post("/", async (req: Request, res: Response) => {
	try {
		...
		if (firstUser !== currentUser.username && secondUser !== currentUser.username) {
...
router.post("/:chatId/messages", async (req: Request, res: Response) => {
	try {
		...
		if (!Types.ObjectId.isValid(chatId)) {
			const resBody: ResponseBody = {
				message: "Invalid id for chat: " + chatId,
				status: ResponseStatus.BAD,
			};
			console.log("Invalid id for chat: " + chatId);
...
router.delete("/:id", async (req: Request, res: Response) => {
	try {
		...
		if (!Types.ObjectId.isValid(id)) {
			const resBody: ResponseBody = {
				message: "Invalid id for delete chat: " + id,
				status: ResponseStatus.BAD,
			};
			console.log("Invalid id for delete chat: " + id);
			return res.status(400).json(resBody);
		}
