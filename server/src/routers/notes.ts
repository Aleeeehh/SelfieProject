...
router.get("/:id", async (req: Request, res: Response) => {
	...
	try {
		...
		const accessList = await getAccessList(foundaccessList.map((x) => x.userId));
...
router.post("/", async (req: Request, res: Response) => {
	try {
        ...
        const createdItemList = await NoteItemSchema.insertMany(
            itemList.map((x) => ({ text: x.text, completed: x.completed, note: createdNote._id }))
        )
...
router.put("/:id", async (req: Request, res: Response) => {
	...
	try {
		...
		if (!inputTitle && !inputText && !inputTags && !inputAccessList && !inputPrivacyStr && !inputItemList) {
			return res.status(400).json({
				status: ResponseStatus.BAD,
				message:
