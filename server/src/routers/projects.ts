...
async function getUserResultFromIdList(idList: string[]): Promise<UserResult[]> {
...
async function getUserResultFromObjectIdList(idList: Types.ObjectId[]): Promise<UserResult[]> {
...
router.get("/", async (req: Request, res: Response) => {
	try {
		...
		const foundProjects = await ProjectSchema.find({
			$or: [{ owner: req.user.id }, { accessList: { $in: [req.user.id] } }],
		}).lean();
		...
		for (const foundProject of foundProjects) {
			...
			for (const foundActivity of foundActivities) {
				...
				const activity: Activity = {
					...
					advancementType: foundActivity.advancementType,
					note,
