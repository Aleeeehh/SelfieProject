...
router.post("/", async (req: Request, res: Response) => {
	try {
		...
		const data = req.body.data as {
			repeatedNotification: boolean;
			repeatTime: number;
			firstNotificationTime: number;
			idEventoNotificaCondiviso: string;
			date: Date;
			frequencyEvent: string;
			isInfiniteEvent: boolean;
			repetitionsEvent: number;
			untilDateEvent: Date;
		};
		...
		if (type === "event") {
			...
			const messageForNotification = message
				.replace(/(\d+)(?=\s+(minuti|ore))/g, displayTime.toString())
		...
		if (data.repeatedNotification) {
			const numberOfRepetitions =
				Math.floor(data.firstNotificationTime / data.repeatTime) - 1;
			console.log(
				"OLTRE ALLA NOTIFICA STESSA, CREO ALTRETTANTE",
				numberOfRepetitions,
				"NOTIFICHE!"
			);
		...
		if (data.repetitionsEvent > 1) {
			...
			for (let i = 2; i <= repetitions; i++) {
				//parti da 2 perchè la prima notifica è già stata creata
				...
				console.log(
					"FIRST NOTIFICATION TIME CHE DOVREBBE ESSERE UGUALE PER TUTTI:",
					data.firstNotificationTime
				);
				...
				if (data.repeatedNotification) {
					...
					for (let j = 1; j <= numberOfRepetitions; j++) {
						// Calcola il nuovo valore di notificationTime per questa iterazione
						console.log(
							"FIRSTNOTIFICATIONTIME iterazione:",
							j,
							"è",
							data.firstNotificationTime
						);
